...
def generate_signals(
    ...
    for i, (_, row) in enumerate(out.iterrows()):
        ...
        if stop_loss_pct is not None and pd.notna(entry_price):
        ...
        if not stop_loss_triggered and pd.notna(upper) and pd.notna(lower):
            if close > upper:
...
def main():
    ...
    if args.sweep or not args.single:
        ...
        print("Dow Version 2 CTA parameter sweep complete.")
    ...
    print("Dow Version 2 CTA backtest complete.")
    print(f"symbol: {symbol}")

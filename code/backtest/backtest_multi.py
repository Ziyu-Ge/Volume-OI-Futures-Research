...
def run_backtest(factor_info):
    ...
    nav_figure_path = os.path.join(
        figure_dir,
        f"{symbol}_{factor_id}_{factor_name}_multi_nav.png"
    )
    ...
    plt.title(f"{symbol} {factor_id} {factor_name} Backtest NAV")
    plt.xlabel("Date")
    plt.ylabel("NAV")
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    print(f"factor {factor_id}: {factor_name} multi backtest complete.")
...
combined_summary_path = os.path.join(
    backtest_dir,
    f"{symbol}_all_factors_long_only_summary.csv"
)

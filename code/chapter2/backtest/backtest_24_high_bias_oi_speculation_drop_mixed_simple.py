...
DEFAULT_OUTPUT_DIR = CHAPTER_RESULTS_DIR / "backtest" / f"{FACTOR_LABEL}_simple"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Simple hourly backtest for factor 24: complete-daily entries "
            "and hourly-composed daily exits."
        )
    )
...
def load_factor_module():
    spec = importlib.util.spec_from_file_location("factor_24_mixed", FACTOR_SCRIPT)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def select_symbols(factor, daily_dir, hourly_dir, symbols):
    all_symbols = factor.discover_symbols(daily_dir, hourly_dir)
    if symbols is None:
        return all_symbols
...
def prepare_backtest_frame(factor, symbol, daily_dir, hourly_dir):
    daily = factor.load_daily(symbol, daily_dir)
    entry_daily = factor.add_complete_daily_entry_features(daily)
    ...
    frame = factor.attach_complete_daily_entry_signal(frame, entry_daily)
    frame = factor.add_hourly_exit_features(frame)
...
def plot_return_summary(summaries, path):
    ...
    if plot_data.empty:
        plot_empty(path, f"{FACTOR_LABEL} total return summary")
        return
...
def run_symbol(factor, symbol, daily_dir, hourly_dir, output_dir):
    frame = prepare_backtest_frame(factor, symbol, daily_dir, hourly_dir)
...
def main():
    ...
    symbols = select_symbols(factor, daily_dir, hourly_dir, args.symbol)
    ...
    print(f"Daily data directory: {daily_dir}", flush=True)
    ...
    print(
        "Summary table: "
        f"{output_dir / 'tables' / 'summary' / f'all_symbols_{FACTOR_LABEL}_simple_summary.csv'}",
        flush=True,
    )
    print(
        "Trade table: "
        f"{output_dir / 'tables' / 'trades' / f'all_symbols_{FACTOR_LABEL}_simple_trades.csv'}",
        flush=True,
    )

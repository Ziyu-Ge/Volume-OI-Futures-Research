...
DEFAULT_OUTPUT_DIR = (
    RESULTS_DIR / "13_price_up_oi_down_all_symbols"
)
...
def save_combined_summaries(symbols, output_dir):
    ...
    if not summaries:
        raise FileNotFoundError("没有找到可汇总的 13 号因子 summary。")
...
def main():
    parser = argparse.ArgumentParser(
        description="运行全部品种的 13 号因子，并输出独立汇总目录。"
    )
    parser.add_argument(
        "--symbols",
        "--symbol",
    ...
    parser.add_argument(
        "--collect-only",
        action="store_true",
        help="不重新运行脚本，只基于结果目录中现有 13 号因子 summary 生成汇总。",
    )
    parser.add_argument(
        "--skip-prepare",
        action="store_true",

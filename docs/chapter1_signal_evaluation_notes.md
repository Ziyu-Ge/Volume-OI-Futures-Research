...
`overall_by_lookahead.csv` 显示，当前 4 个因子合并后，3、5、10 日窗口均显著高于基准胜率。回撤分布列来自事件明细中可评价样本的 `strategy_observation_max_drawdown`，即窗口内最大观察回撤的分位数。

| 观察窗口 | 可评价 / 命中 | 平均阈值 | 策略胜率 | 基准胜率 | 胜率差 | 观察最大回撤 P30 | P60 | P90 | 最大 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 3 日 | 2401 / 1099 | 1.23% | 45.77% | 31.29% | +14.49 pct | 0.59% | 1.93% | 4.76% | 29.52% |
| 5 日 | 2399 / 1180 | 1.23% | 49.19% | 35.34% | +13.85 pct | 0.95% | 2.64% | 6.26% | 29.52% |
| 10 日 | 2388 / 1287 | 1.23% | 53.89% | 39.53% | +14.36 pct | 1.64% | 3.85% | 8.39% | 42.34% |

解读：

- 三个窗口的胜率差集中在 `+13.85` 到 `+14.49` 个百分点，信号对未来收盘回落有稳定正贡献。
- 10 日窗口的绝对胜率最高，为 `53.89%`；3 日窗口的相对基准优势略高，为 `+14.49` 个百分点。
- 观察窗口越长，窗口内最大回撤分布整体右移：60 分位从 3 日的 `1.93%` 升至 10 日的 `3.85%`，90 分位从 `4.76%` 升至 `8.39%`。
- 这更像“后续数日回落风险提示”信号，不是一个可以直接对应当天或隔日交易执行的系统。需要配合别的信号。

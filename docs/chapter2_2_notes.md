...
这样不同品种、不同价格量级可以放在同一个阈值下比较。例如 `R(C,2,d)<=-0.00475` 表示 2 日价格斜率相对 2 日均价的比例必须不高于 `-0.475%`。

当 `n=2` 时，居中横坐标是 `[-0.5,0.5]`，所以 2 日斜率可以简化为：

```text
beta(X,2,d) = X_d - X_{d-1}
```
...
也就是说，23/24 号因子要求 `log(成交量/持仓量)` 这条线在各自窗口内往下走。
...
`vol_d` 用在平仓线上。代码中 `m` 来自 `volatility_window`：21-23 号固定为 `10`，24 号为 `7`，当前 rolling 候选值为 `5、7、10、14`。
...
筛选含义：使用 9/12/90 日均线，要求持仓和价格在 2 日窗口内回落，同时要求投机度的 30 日趋势向下。
...
| 24 | 小时 | 9/12/90 | -0.014 | -0.10 | `R(OI,2)<=-0.01225` | `R(C,2)<=-0.00475` | `beta(S,30)<=-0.0055` |
...
因此，不同字段的非基准值不会互相组合。当前共评估 `44` 组候选参数；表中同时列出基准值和单字段变化时可取的值。
...
| `B1下限` | -0.020, -0.017, -0.014, -0.011, -0.008 |
| `B2下限` | -0.14, -0.12, -0.10, -0.08, -0.06 |
| `oi_slope_window` | 2, 3, 5 |
| `oi_slope_threshold` | -0.018, -0.015, -0.01225, -0.010, -0.0075 |
| `close_slope_window` | 2, 3, 5 |
| `close_slope_threshold` | -0.007, -0.006, -0.00475, -0.0035, -0.0025 |
| `speculation_slope_window` | 24, 30, 36 |
| `speculation_slope_threshold` | -0.009, -0.007, -0.0055, -0.004, -0.002 |
...
| --- | ---: | --- |
| 基准参数 | 0 | — |
| `speculation_slope_threshold` | 4 | 取 `-0.002` |
| `ma_trend` | 1 | 取 `120` |
| `B2下限` | 2 | 取 `-0.08` |
| `ma_long` | 2 | 取 `10` |
| `ma_short` | 1 | 取 `10` |
| `oi_slope_threshold` | 1 | 取 `-0.018` |
| `B1下限` | 1 | 取 `-0.008` |
| `trailing_multiplier` | 3 | 取 `2.80` 或 `3.20` |
...
| 24 | 11.20% | -6.24% | 1.27 | 1490 |
| 24 rolling | 11.21% | -6.60% | 1.22 | 1534 |
